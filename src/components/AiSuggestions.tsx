      ...
      const fetchSuggestions = async () => {
        ...
        if (note) {
          ...
          try {
            ...
            setError("Oops! Failed to load AI insights. Please try again later or select another note.");
          } finally {
            setIsLoading(false);
          }
        } else {
  ...
  return (
    <Card className="h-full flex flex-col shadow-lg rounded-lg bg-card text-card-foreground border border-border/80">
      <CardHeader className="border-b border-border/70 pb-3 pt-4 px-5">
        ...
        {currentNoteTitle && (
            <CardDescription className="text-xs text-muted-foreground pt-1 pl-0.5 truncate" title={`Insights related to: "${currentNoteTitle}"`}>
                Related to: <span className="font-medium text-foreground/80">"{currentNoteTitle}"</span>
            </CardDescription>
        )}
      </CardHeader>
      <ScrollArea className="flex-1 custom-scrollbar"> {/* Added custom-scrollbar */}
        <CardContent className="p-5">
          {isLoading && (
            <div className="flex flex-col items-center justify-center space-y-3.5 py-10 text-muted-foreground">
              ...
              <p className="text-xs text-center">This may take a few moments.</p>
            </div>
          )}
          {error && (
            <div className="flex flex-col items-center justify-center space-y-3.5 py-10 text-destructive">
              <AlertTriangle className="h-8 w-8" />
              <p className="text-sm font-medium text-center">Error Loading Insights</p>
          ...
          {!isLoading && !error && !suggestions && !selectedNoteIdForAI && (
            <div className="text-center text-muted-foreground py-10 px-6 space-y-4">
              ...
              <p className="text-sm font-medium">Unlock AI Insights</p>
              <p className="text-xs leading-relaxed">
                Select a note and click the <Sparkles className="inline h-3.5 w-3.5 mx-0.5 text-primary relative -top-px" /> icon 
                on it to discover related ideas and suggestions powered by AI.
              </p>
            </div>
          )}
          {!isLoading && !error && suggestions && suggestions.relatedNotes.length === 0 && selectedNoteIdForAI && (
             <div className="text-center text-muted-foreground py-10 px-6 space-y-4">
              ...
              <p className="text-sm font-medium">No Specific Insights Found</p>
              <p className="text-xs leading-relaxed">
                The AI couldn't find any specific related notes or insights for "{currentNoteTitle}" at this time. Try a different note or add more details to this one.
              </p>
            </div>
          )}
