  ...
  return (
    <div className="flex flex-col h-screen bg-background text-foreground">
      <header className="p-5 border-b border-border/80 bg-card shadow-sm sticky top-0 z-10">
        <div className="container mx-auto flex items-center justify-between gap-x-4 md:gap-x-6">
          <div className="flex items-center gap-2.5 shrink-0">
            <svg role="img" aria-label="NoteSphere Logo" className="h-8 w-8 md:h-9 md:w-9 text-primary" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            ...
            <h1 className="text-2xl md:text-3xl font-semibold text-foreground tracking-tight">NoteSphere</h1>
          </div>
      ...
      <main className="flex-1 flex overflow-hidden container mx-auto py-6 md:py-8 gap-6 md:gap-8">
        ...
        <aside className={`hidden md:block w-[360px] lg:w-[400px] transition-all duration-300 ease-in-out overflow-y-auto rounded-lg custom-scrollbar ${isAiPanelOpen ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full w-0 pointer-events-none'}`}>
