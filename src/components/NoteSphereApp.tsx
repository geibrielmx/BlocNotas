...
import { Download, PlusCircle, Search, Sparkles } from 'lucide-react';
import { convertNotesToCsv, downloadTextFile } from '@/lib/note-utils';
import { useToast } from "@/hooks/use-toast";
import {
  Sheet,
  SheetContent,
  SheetTrigger, // Corrected import
...
export function NoteSphereApp() {
  ...
  return (
    <div className="flex flex-col h-screen bg-background text-foreground">
      <header className="p-6 border-b border-border bg-card shadow-md">
        <div className="container mx-auto flex items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            <svg role="img" aria-label="NoteSphere Logo" className="h-9 w-9 text-primary" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            ...
            <h1 className="text-3xl font-bold text-foreground">NoteSphere</h1>
          </div>
          <div className="flex-1 min-w-0 max-w-lg">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-11 w-full bg-background border-input focus:ring-primary rounded-md shadow-sm h-11"
                value={searchTerm}
          ...
          <div className="flex items-center gap-3">
            <Button onClick={handleAddNewNote} variant="default" size="lg" className="shadow-md">
              <PlusCircle className="mr-2 h-5 w-5" />
              Add Note
            </Button>
            <Button onClick={handleExportNotes} variant="outline" size="lg" className="shadow-sm">
              <Download className="mr-2 h-5 w-5" />
              Export
            </Button>
             <Sheet>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className="md:hidden h-11 w-11" aria-label="Toggle AI Panel">
                  <Sparkles className="h-6 w-6" />
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="w-[320px] sm:w-[420px] p-0 bg-card border-l border-border">
      ...
      <main className="flex-1 flex overflow-hidden container mx-auto py-8 gap-8">
        <div className="flex-1 overflow-y-auto pr-3 scroll-smooth rounded-lg"> {/* Added pr-3 for scrollbar spacing and rounded-lg */}
          <NoteList onEditNote={handleEditNote} />
        </div>
        <aside className={`hidden md:block w-[380px] lg:w-[420px] transition-all duration-300 ease-in-out overflow-y-auto rounded-lg ${isAiPanelOpen ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full w-0'}`}>
